...
//    }
    ...
    public static SavedProfileDto transformSavedProfileEntityToDto(SavedProfileEntity savedProfileEntity){
        // Assuming that savedProfileEntity.getSavingUser() and savedProfileEntity.getSavedUser() are not null.
        return new SavedProfileDto(
                savedProfileEntity.getId(),
                savedProfileEntity.getSavingUser().getUserId(),
                savedProfileEntity.getSavedUser().getUserId()
        );
    }
...
//        SavedProfileEntity savedProfileEntity = new SavedProfileEntity();
//        // If the ID is generated by the database, you might not need to set it here
//        UserEntity savingUser = usersRepo.findById(savedProfileDto.getSavingUserId())
//                .orElseThrow(() -> new EntityNotFoundException("Saving user not found"));
//        UserEntity savedUser = usersRepo.findById(savedProfileDto.getSavedUserId())
//                .orElseThrow(() -> new EntityNotFoundException("Saved user not found"));
//        savedProfileEntity.setSavingUser(savingUser);
//        savedProfileEntity.setSavedUser(savedUser);
//
//        return savedProfileEntity;
//    }
    ...
    public static SavedProfileEntity transformSavedProfileDtoToEntity(SavedProfileDto savedProfileDto, UsersRepo usersRepo){
        SavedProfileEntity savedProfileEntity = new SavedProfileEntity();
        // If the ID is generated by the database, you might not need to set it here
        UserEntity savingUser = usersRepo.findById(savedProfileDto.getSavingProfileId()).orElseThrow(() -> new EntityNotFoundException("User not found"));
        UserEntity savedUser = usersRepo.findById(savedProfileDto.getSavedProfileId()).orElseThrow(() -> new EntityNotFoundException("User not found"));
        savedProfileEntity.setSavingUser(savingUser);
        savedProfileEntity.setSavedUser(savedUser);
